...
extern "C"
jdouble Java_org_fmod_racobox_util_AudioUtilNative_processSampleData(JNIEnv* env,jobject,
        jbyteArray bufferParam,
        jint sampleRateParam){
    jsize sampleLength = env->GetArrayLength(bufferParam);
    jbyte* localSample = env->GetByteArrayElements(bufferParam, nullptr);
    double padSample[sampleLength/2];
    unsigned int i;
    // big to little endian and hanning
    for(i=0;i<sampleLength;i+=2){
        padSample[i >> 1u] =  (short)(((unsigned)localSample[i]&0xFFu) | (((unsigned)localSample[i+1]&0xFFu) << 8u));
        padSample[i >> 1u] =  singleHanning(padSample[i >> 1u],i >> 1u,sampleLength/2);
    }
    jdouble result = newFFT(padSample,sampleLength/2,sampleRateParam);
    env->ReleaseByteArrayElements(bufferParam,localSample,0);
    return result;
}
